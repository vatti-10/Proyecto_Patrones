...
public class ConvertidorInputsDamas implements IConvertidorInput{
    ...
    private int coordenadaY(String pString) {
        ...
        if(valor <= 5) return 0;
        if(valor > 5 && valor <= 10) return 1;
        if(valor > 10 && valor <= 15) return 2;
        if(valor > 15 && valor <= 20) return 3;
        if(valor > 20 && valor <= 25) return 4;
        if(valor > 25 && valor <= 30) return 5;
        if(valor > 30 && valor <= 35) return 6;
        if(valor > 35 && valor <= 40) return 7;
        if(valor > 40 && valor <= 45) return 8;
        if(valor > 45) return 9;
        return -1;
    }
